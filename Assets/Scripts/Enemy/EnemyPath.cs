...
public class EnemyPath : MonoBehaviour
{
    ...
    private void FindAllNodes()
    {
        ...
        foreach (Transform node in this.transform.GetComponentInChildren<Transform>())
        {
            ...
            this._nodes.Add(new EnemyPathNode() { Index = i, Position = node.position, WaitTime = node.GetComponent<EnemyPathNodeCom>().Node.WaitTime, Id = Random.Range(0, int.MaxValue), RotationSpeed = node.GetComponent<EnemyPathNodeCom>().Node.RotationSpeed});
